...
//		update->ip[3] = 105;
	if (port == 0)
		port = 80;
	update->port = port;
	os_printf("[OTA]Server "IPSTR":%d. Path: [%s]\n", IP2STR(update->ip), update->port, ota_path);
...
void ICACHE_FLASH_ATTR user_ota_start(char *s) {
	...
	uint16_t pos = 0;	//http前缀字符串位置
	int16_t pos1 = 0;	//域名/端口字符串位置
	int16_t pos2 = 0;		//仅域名结束位置
	uint8_t i, j;
	...
	os_strncpy(domain, s + pos, pos2 - pos);
